...
void syscallReadStdIn(struct StackFrame *sf) {
	...
	if (dev[STD_IN].value == 0)
	{
		...
		int sel = sf->ds;
		char *str = (char*)sf->edx;
		int max = sf->ebx; // MAX_BUFFER_SIZE, reverse last byte
		int i = 0;
		char character = 0;
		asm volatile("movw %0, %%es"::"m"(sel));
		for(i = 0; i < max - 1; i++)
...
void syscallSemWait(struct StackFrame *sf) {
	...
	if(sem[i].value <= 0)//need be bolcked
	{
		sem[i].value--;
		pcb[current].state = STATE_BLOCKED;
		pcb[current].sleepTime = -1; 
		...
		pcb[current].regs.eax = 0; //return value
	}
	else //normally
	{
		sem[i].value--;
...
void syscallSemPost(struct StackFrame *sf) {
	...
	if(sem[i].value < 0)//release a process
	{
		sem[i].value++;
